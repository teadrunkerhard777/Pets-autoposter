...
A separate workflow selects at most one short vertical animal video from the
official Pexels API at 13:00 and 20:00 Asia/Yekaterinburg. It never repeats a
video already present in publication history, credits the creator and Pexels,
and skips the slot when no suitable unpublished video is available. It requires
the free `PEXELS_API_KEY` secret; see [DEPLOYMENT.md](DEPLOYMENT.md).
