...
class VideoResponse:
    def __init__(self, chunks, content_type="video/mp4", size=None):
        self.headers = {"Content-Type": content_type}
        if size is not None:
            self.headers["Content-Length"] = str(size)
        self.chunks = chunks
...
def test_video_download_validates_mp4_and_removes_temporary_file(monkeypatch):
    ...
    result = download_video_temp("https://video.test/pet.mp4", 1_000)
...
def test_video_download_rejects_non_mp4_bytes(monkeypatch):
    ...
    with pytest.raises(VideoDownloadError, match="do not match MP4"):
        download_video_temp("https://video.test/pet.mp4", 1_000)
